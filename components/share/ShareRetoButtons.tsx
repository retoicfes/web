...
export function ShareRetoButtons({ puntaje, apodo, colegio, destacado = false, className = "" }: Props) {
  ...
  const url = useMemo(() => getShareUrl(), []);

  const abrir = useCallback((href: string) => {
    window.open(href, "_blank", "noopener,noreferrer");
  }, []);

  const copiar = useCallback(async () => {
    const ok = await copyShareText(mensaje);
    if (ok) {
      setCopiado(true);
  ...
  const btnSecundario =
    "flex w-full items-center justify-center gap-2 rounded-2xl py-3.5 font-semibold text-white transition active:scale-[0.98]";
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      <button
        type="button"
        onClick={() => abrir(whatsAppShareUrl(mensaje))}
        className={`${btnPrimario} bg-[#25D366] shadow-[#25D366]/30 ${destacado ? "text-lg" : ""}`}
      >
      ...
      <button
        type="button"
        onClick={() => abrir(telegramShareUrl(url, buildShareTextBody(ctx)))}
        className={`${btnSecundario} bg-[#229ED9] shadow-md shadow-[#229ED9]/25`}
      >
        <IconTelegram className="h-6 w-6 shrink-0" />
